...
<div class="dashboard-card mb-6">
    ...
    <div class="flex flex-col gap-3">
        <div>
            <h3 class="text-subheading">Total USD Balance</h3>
...
// Fetch user wallets
$userWallets = UserWallet::where('user_id', auth()->id())->get();

// Map symbols to CoinGecko IDs (all your coins)
$symbolToId = [
   'BTC'  => 'bitcoin',
'ETH'  => 'ethereum',
'USDT' => 'tether',
...
$ids = implode(',', $symbolToId);
$prices = Http::get("https://api.coingecko.com/api/v3/simple/price?ids={$ids}&vs_currencies=usd")->json();

$totalUsd = 0;
@endphp

<div>
    <h3 class="text-subheading">Your Wallet</h3>
    <ul>
        @forelse ($userWallets as $wallet)
            @php
                $id = $symbolToId[$wallet->currency] ?? null;
                $usdPrice = $id ? ($prices[$id]['usd'] ?? 0) : 0;
                $usdValue = $wallet->balance * $usdPrice;
                $totalUsd += $usdValue;
            @endphp
            <li>
                {{ $wallet->currency }}: {{ $wallet->balance }} = ${{ number_format($usdValue, 2) }}
            </li>
        @empty
            <li>No crypto wallets found.</li>
        @endforelse
    </ul>

    <h3 class="text-subheading mt-4">Crypto Portfolio (USD Value)</h3>
    <p class="text-heading text-xl md:text-2xl" style="color: #00A383;">
...
async function updateBuyCryptoFields() {
    ...
    let iconUrl = selectedOption.getAttribute('data-icon');

    // Fallback if iconUrl is missing or empty
    if (!iconUrl || iconUrl.trim() === '') {
        iconUrl = `https://raw.githubusercontent.com/spothq/cryptocurrency-icons/master/svg/color/${symbol.toLowerCase()}.svg`;
    }
    icon.src = iconUrl;
